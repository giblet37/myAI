...
import datetime
from functions.instant_time import InstantTime
from functions.time_interval import TimeInterval
...
class IntentParser:
    ...
    def get_intent_name(payload):
        ...
        if 'intent' in payload and 'intentName' in payload['intent']:
            ...
            return payload['intent']['intentName'].split(':')[-1].split(":")[-1]
        return None
    ...
    def parse_instant_time(slot):
        ...
        if not date:
            return None
        grain = IntentParser.get_dict_value(slot,
                                        ['value', 'grain'])
    ...
    @staticmethod
    def parse_date_value(payload, slot_name):
        """ Parse a slot into a TimeInterval object.
        ...
        kind = IntentParser.get_dict_value(slot, ['value', 'kind'])
        if kind == "InstantTime":
            grain = IntentParser.get_dict_value(slot, ['value', 'grain'])
            startDate = IntentParser.parse_instant_time(slot).datetime
            ...
            if grain == 'Day':
                ...
                endDate = startDate + datetime.timedelta(months=1)
            elif grain == 'Year':
                endDate = startDate + datetime.timedelta(years=7)
            return {"start":startDate,"end":endDate}
        elif kind == "TimeInterval":
            return {"start":IntentParser.parse_time_interval(slot).start,"end":IntentParser.parse_time_interval(slot).end}

        return None #must not have been a datetime type

    @staticmethod
    def parse_date_range(payload, slot_name):
        """ Parse a slot into a TimeInterval object.
    ...
    @staticmethod
    def parse_date_raw_value(payload, slot_name):
        """ Parse a slot into a TimeInterval object.
        ...
        kind = IntentParser.get_dict_value(slot, ['value', 'kind'])
        if kind == "TimeInterval" or kind == "InstantTime":
            return  IntentParser.get_dict_value(slot, ['rawValue'])
